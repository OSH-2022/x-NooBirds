...
CarSim 是 Mechanical Simulation 公司开发的强大的动力学仿真软件，被世界各国的主机厂和供应商所广泛使用。 CarSim 针对四轮汽车， 轻型卡车。
...
Apollo是由百度-阿波罗开发的高性能框架，用于加速自动驾驶汽车的开发、测试和部署。同时该计划还包括了一系列仿真平台、硬件开发套件等。

Apollo已经开放了超过22万行代码，形成了开源开发生态。
