...
// semaphores for data passing between the object tracker and the scheduler
...
double v_on_car[N] = {0.6, 0.6, 0.6};   // corresponds to throttle values on the car

// data structure providing a pipe to the remote script on the car
// the pipe is implemented via SSH running as subprocess
...
// scheduling method #1
// car runs on fixed route;
// the scheduler controls the velocity of the car
void *thread_sched_vel_factor(void *args) {
    ...
    while (true) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            // 0.6 is the maximum speed for the car to follow the line reliably
